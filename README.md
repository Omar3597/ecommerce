...
The application uses a **modular architecture**. Each feature domain is self-contained under `src/modules/`. A separate **worker process** (`worker.server.ts`) runs alongside the API, consuming BullMQ jobs from Redis-backed queues.
